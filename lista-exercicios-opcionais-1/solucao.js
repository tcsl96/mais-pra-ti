...
function criarMatriz(linhas, colunas=undefined, valorMaximo=5) {
    ...
    if (colunas === undefined) { colunas = linhas; }
...
function questao3() {
    ...
    if (distancia > 200) { precoQuilometro = 0.45; }
...
/**
 * 10. Crie um programa usando a estrutura “faça enquanto” que leia vários números.  
...
function questao10() {
    ...
    do {
        ...
        somaNumeros += numero;  // a)

        if (menorNumero === undefined) {
            menorNumero = numero;
        } else {
            if (numero < menorNumero) { menorNumero = numero; }  // b)
        }

        if (numero % 2 == 0) { numerosPares++; }  // d)
        qtdNumeros++;
...
/**
 * 13. Crie um programa que preencha automaticamente (usando lógica,  
 * não apenas atribuindo diretamente) um vetor numérico com 15 posições  
...
/**
 * 19. Escrever um programa para ler 5 horários.  
...
/**
 * 21. Faça uma função que recebe, por parâmetro, a altura (alt) e o sexo de uma pessoa e retorna o seu peso ideal.  
 * Calcular o peso ideal usando a fórmula:  
 * - Para homens peso ideal = 72.7 x alt - 58;  
...
function questao21() {
    ...
    if (sexo == 1) { pesoIdeal = 72.7 * altura - 58; }
...
function questao22() {
    ...
    while (true) {
        ...
        if (salario <= 350) { salarioMenor350 += 1; }
        qtdPessoas++;
...
/**
 * 23. Criar e imprimir a matriz identidade MI[1..7,1..7] em que todos  
 * os elementos da diagonal principal são iguais a 1 e os demais são nulos.
 */
function questao23() {
...
function questao24() {
    ...
    for (let i = 0; i < matrizM.length; i++) {
        ...
        for (let j = 0; j < matrizM[i].length; j++) {
            ...
            if (numeroAleatorio == 0) { matrizM[i][j] = 0; }
            else { matrizM[i][j] = -1; }
        }
    }
    ...
    for (let i = 0; i < matrizM.length; i++) {
        contNegativos = 0;
        for (let j = 0; j < matrizM[i].length; j++) {
            if (matrizM[i][j] < 0) { contNegativos++; }
        }
...
/**
 * 25. Faça um algoritmo que leia uma matriz de 15 X 20 de números reais e  
 * mostre a soma de cada coluna separadamente.
 */
...
/**
 * 30. Escrever um algoritmo que lê uma matriz M(5,5) e cria 2 vetores SL(5) e SC(5)  
 ...
 * Conte quantos valores iguais a A estão na matriz. Crie, a seguir,  
 * uma matriz X contendo todos os elementos de V diferentes de A. Mostre os resultados.
 */
...
/**
 * 32. Escrever um algoritmo que lê uma matriz M(12,13) e  
 * divida todos os 13 elementos de cada uma das 12 linhas de M  
 * pelo maior elemento em módulo daquela linha.  
...
function questao32(matriz) {
    ...
    for (let i = 0; i < matriz.length; i++) {
        ...
        for (let j = 1; j < matriz[i].length; j++) {
            ...
            if (elemento > maiorElemento) { maiorElemento = elemento; }
        }
